...
def toggle_availability(item_id, is_available):
    ...
    try:
        ...
        conn.commit()
        status_map = {1: "AVAILABLE",0 : "INACTIVE"}
        print(f"✅ Item ID {item_id} status set to {status_map.get(is_available, 'UNKNOWN')}.")
    finally:
        conn.close()


def get_orders_by_token(token_number):
    """
    Finds ACTIVE orders for the current day matching the token.
    Filters out old/expired tokens to prevent collisions.
    """
    from core.db_config import get_db_connection
    conn = get_db_connection()
    try:
        cursor = conn.cursor(dictionary=True)
        # We search for: Correct Token + Pending Status + Not Expired + Today only
        query = """
            SELECT o.order_id, o.token_number, u.username, o.total_amount, o.expiry_time,
                   GROUP_CONCAT(CONCAT(m.item_name, ' (x', oi.quantity, ')') SEPARATOR ', ') as items_list
            FROM orders o
            JOIN users u ON o.user_id = u.user_id
            JOIN order_items oi ON o.order_id = oi.order_id
            JOIN menu m ON oi.item_id = m.item_id
            WHERE o.token_number = %s 
        ...
        return cursor.fetchall() # Returns a list in case of rare duplicates
    finally:
        conn.close()

def mark_order_delivered(order_id):
    """
    Fixed: Uses 'completed' status to match database ENUM.
    """
    from core.db_config import get_db_connection
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        # Changed 'delivered' to 'completed'
